...
async def register_referral(referrer_id: int, referred_user_id: int) -> bool:
    ...
    existing_user = await pool.fetchval("""
        ...
        UNION
        SELECT 1 FROM business_connections WHERE user_id = $1 OR user_chat_id = $1
        UNION
...
async def get_user_subscription_status(user_id: int, admin_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    if not expires_at_val:
        return {
            ...
            "expires_at": None,
            "is_trial_used": bool(row["is_trial_used"])
        }
...
async def add_subscription(user_id: int, days: int, plan_type: str, stars_paid: int) -> str:
    ...
    if row and row["expires_at"]:
        try:
            ...
            exp = datetime.strptime(clean_str, "%Y-%m-%d %H:%M:%S")
            if exp > datetime.now():
                current_expiry = exp
        except Exception:
            pass
...
async def is_user_business_connected(user_id: int) -> bool:
    ...
    try:
        pool = await get_pg_pool()
        val = await pool.fetchval(
            "SELECT is_enabled FROM business_connections WHERE (user_id = $1 OR user_chat_id = $1) ORDER BY updated_at DESC LIMIT 1",
            user_id
        )
